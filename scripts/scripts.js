...
function populateUsers() {
  ...
  accountsObject.forEach(element => {
    $('#usersTable').append('<tr><td onclick="changePass(\''+ element.account_id +'\')">'+element.account_username+'</td><td><div class="chip" onclick="changeGroup(\''+ element.account_id +'\', \''+element.account_group+'\')">'+element.account_group+'<i class="close material-icons">edit</i></div></td><td><div class="chip" onclick="changeConcediu(\''+ element.account_id +'\', \''+element.zile_concediu+'\')">'+element.zile_concediu+'<i class="close material-icons">edit</i></div><div class="chip" onclick="changeRaport(\''+ element.account_id +'\', \''+element.zile_report+'\')">'+element.zile_report+'<i class="close material-icons">edit</i></div></td><td><label><input type="checkbox" id="userNo_' + element.account_id + '" onclick="changeUserState(this.id)"' + (element.account_enabled == "1" ? 'checked="checked" ' : '') +' /><span></span></label></td></tr>');
  });
}
