...
function addProject (array $basicData) {
    ...
    /* Values array for PDO */
    $values = array(':name' => $basicData[0], ":client_id" => (int)$basicData[1], ":type_id" => (int)$basicData[2], ":external" => (int)$basicData[3], ":budget" => (int)$basicData[4], ":start_date" => date("Y-m-d", strtotime($basicData[5])), ":deadline" => date("Y-m-d", strtotime($basicData[6])));
    /* Execute the query */
    try
    {
        $res = $pdo->prepare($query);
